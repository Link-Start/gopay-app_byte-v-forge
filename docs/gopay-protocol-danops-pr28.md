...
- GoPay app device headers keep the PR #28 captured app/profile shape, but generated signup devices now default to Apple/iOS identity: `X-Platform=iOS`, `X-DeviceOS=iOS, ...`, Apple phone model/screen profile, APNs-style `X-DeviceToken`, and Safari iOS TLS profile. Per-device persisted identity and lower hardware/network IDs (`x-uniqueid`, `D1`, WiFi BSSID/SSID, `m1_connection_id`, `m1_signature`, `m1_signature_time`, `m1_device_uuid`, AppsFlyer, Firebase app instance ID, AD ID/App Set ID metadata, `X-IMEI`, and `X-IpAddress`) are still generated per profile. Use `GOPAY_STATIC_DEVICE_IDENTITY=1` only for deterministic protocol debugging; do not use it for signup/payment flows because a globally reused device identity is easy to correlate.
...
- `scripts/gopay-adb-device-profile.sh` is kept only for focused Android replay/debugging. New signup/account device generation defaults to Apple/iOS unless explicitly overridden by environment.
...
- PR #28 notes do not mention TLS/JA3/ClientHello details. They only mention server-side device fingerprinting and `x-m1`/device fields. The app now binds one Safari iOS TLS profile when a device/proxy state is generated and reuses it for that state. Useful envs:
  - `GOPAY_TLS_PROFILE=safari_ios_18_5` pins a profile.
  - `GOPAY_TLS_PROFILES=safari_ios_18_5,safari_ios_18_0` limits the random pool.
