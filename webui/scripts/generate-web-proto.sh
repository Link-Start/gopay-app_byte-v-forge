...
PROTO_DIR="${PROTO_DIR:-${SOURCE_ROOT}/gopay-app/proto}"
...
if [[ -z "${PLUGIN}" || ! -x "${PLUGIN}" ]]; then
  printf 'ts-proto plugin not found; run npm install in gopay-app/webui or webui first\n' >&2
  exit 1
fi
